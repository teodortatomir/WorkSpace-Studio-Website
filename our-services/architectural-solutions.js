...
const animatedSections = document.querySelectorAll(
  ".arch-hero-copy, .arch-hero-image, .arch-section-head, .arch-system-card, .arch-focus-image, .arch-focus-copy, .arch-cta-panel"
);
...
animatedSections.forEach((element, index) => {
  ...
  const initialTransform =
    direction === 0
      ? "translate3d(-28px, 40px, 0) scale(0.985)"
      : direction === 1
...
const hoverCards = document.querySelectorAll(".arch-system-card, .arch-focus-copy, .arch-cta-panel");

hoverCards.forEach((card) => {
  card.addEventListener("mouseenter", () => {
    card.style.transform = "translateY(-6px)";
    card.style.boxShadow = "0 24px 56px rgba(46, 33, 24, 0.14)";
  });
...
const tiltCards = document.querySelectorAll(".arch-hero-image, .arch-focus-image");
...
tiltCards.forEach((card) => {
  ...
  card.addEventListener("mousemove", (event) => {
    ...
    card.style.boxShadow = "0 28px 60px rgba(46, 33, 24, 0.16)";
  });
...
const parallaxImages = document.querySelectorAll(".arch-hero-image img, .arch-focus-image img");
